...
Fenetre6::Fenetre6(const QDate& d){
    ...
    for(vector<Evenement*>::const_iterator eit = e.getEvenement().begin(); eit != e.getEvenement().end(); ++eit){
        ...
        const QDate& dateEvenement = (*eit)->getDate();

        if (dateEvenement>=d && dateEvenement<=d.addDays(7)){

            QTableWidgetItem *newItem = new QTableWidgetItem(tr("1"));

            //On rajoute un 1 à l'heure de début quelque soit le type
            if(dateEvenement.addDays(-d.day()).day()==31)
                tab->setItem((*eit)->getHoraire().getHeure(),0, newItem);
            else tab->setItem((*eit)->getHoraire().getHeure(),dateEvenement.addDays(-d.day()).day(), newItem);
           ...
           if(typeid(**eit)==typeid(ProgrammationTache)){
                    ...
                    unsigned int j=0;
                    unsigned int hdebutEvenement = (*eit)->getHoraire().getHeure();
                    unsigned int jourTab = dateEvenement.addDays(-d.day()).day();
                    for(unsigned int i=1 ; i<dynamic_cast<ProgrammationTache*>(*eit)->getDuree().getHeure();i++){
                        QTableWidgetItem *newItem = new QTableWidgetItem(tr("1"));
                        if(jourTab==31) jourTab=0;
                           // tab->setItem(hdebutEvenement+i,0, newItem);
                        //else
                            tab->setItem(hdebutEvenement+i,jourTab+j, newItem);
                        qDebug()<<"i vaut :";
                        qDebug()<<i;
                        qDebug()<<hdebutEvenement+i;
                        qDebug()<<jourTab+j;
                        if((hdebutEvenement+i!=0) &&((hdebutEvenement+i)%23==0)) {
